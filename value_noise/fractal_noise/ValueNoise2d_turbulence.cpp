...
float fractalNoise(Vec2f x){
  ...
  for (unsigned i = 0; i < numLayers; i++) {
    noiseSum += std::fabs(2 * noise.eval(x * frequency) - 1) * amplitude;
...
int main(int argc, char const *argv[]) {
  ...
  for (unsigned j = 0; j < ny; j++) {
    for (unsigned i = 0; i < nx; i++) {
      value[j*nx +i] = fractalNoise(Vec2f(i,j)*frequency);
      maxValue = std::fmax(maxValue, value[j*nx + i]);
    }
  }
  ...
  for (unsigned j = 0; j < ny; j++) {
    for (unsigned i = 0; i < nx; i++) {
      ir = ig = ib = int(255.99 * value[j*nx +i] / maxValue);
      cout << ir << " " << ig << " " << ib << "\n";
    }
  }
